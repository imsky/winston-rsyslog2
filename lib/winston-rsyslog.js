...
var Rsyslog = exports.Rsyslog = function (options) {
  ...
  if (['U', 'U4', 'U6', 'T'].indexOf(this.protocol) === -1) {
    throw new Error('Undefined Protocol! (valid options are U, U4, U6 or T)');
  }
...
Rsyslog.prototype.log = function (level, msg, meta, callback) {
  ...
  var _buffer = Buffer.from('<' + _pri + '>' + _date + ' ' + this.hostname + ' ' + this.tag + ' ' + _message);

  if (this.protocol === 'U4' || this.protocol === 'U6') {
...
function sendUdp(self, buffer, callback) {
  var client = dgram.createSocket((self.protocol === 'U6') ? 'udp6' : 'udp4');
